...
exports.processTemplates = function (name, dir, type, that, defaultDir, configName, module) {
  ...
  _.chain(fs.readdirSync(templateDirectory))
    ...
    .each(function (template) {
      ...
      exports.inject(path.join(dir, customTemplateName), that, module, name);
    });
};
...
exports.injectNavRoute = function (moduleFile, uirouter, name, route, routeUrl, that) {

  var code = '<li><a ui-sref="' + name + '" ui-sref-active="active">' + _.camelize(_.classify(name)) + '</a></li>';
  exports.addToFile('index.html', code, exports.ROUTE_MARKER_NAV);
...
exports.injectRoute = function (moduleFile, uirouter, name, route, routeUrl, that) {
  ...
  // Basically only use ui router (but we'll keep the choice for now)
  if (uirouter) {
    var code = '$stateProvider.state(\'' + name + '\', { url: \'' + route + '\', templateUrl: \'' + routeUrl + '\', data: { pageTitle: \'' + name + '\' } });';
